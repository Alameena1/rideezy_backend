...
export class TrackingService implements ITrackingService {
  constructor(
    ...
    console.log("[TrackingService] Initialized with trackingRepo, rideRepo, and osrmClient");
  }
  ...
  async updateTrackingPosition(rideId: string, position: [number, number]): Promise<void> {
    ...
    await retry(async (bail, attempt) => {
      console.log(`[TrackingService] Attempt ${attempt} to update tracking position for ride ${rideId}`);
      ...
      try {
       let session = await this.trackingRepo.startSession();
        ...
        await session.withTransaction(async () => {
          ...
          console.log(`[TrackingService] Updated tracking position for ride ${rideId} to ${position} in ${Date.now() - startTime}ms`);
        });
      } catch (error: unknown) {
        ...
        if (session) {
          try {
            ...
            console.error("[TrackingService] Failed to end session:", endSessionError);
          }
        }
      }
    }, {
...
async updateTrackingAction(rideId: string, passengerId: string, action: "picked" | "dropped"): Promise<void> {
    const session = await this.trackingRepo.startSession();
    try {
      await session.withTransaction(async () => {
        const ride = await this.rideRepo.findOne({ _id: new Types.ObjectId(rideId) }, { session });
        ...
        const updateField = action === "picked" ? "pickupActions" : "dropoffActions";
        const actions = tracking[updateField].map((a) =>
          a.passengerId === passengerId ? { ...a, status: "Completed" } : a
        );

        const status = actions.every((a) => a.status === "Completed") ? "Started" : "Paused";
        await this.trackingRepo.updateTracking(
          ride._id,
          { [updateField]: actions, status },
          { session }
        );
        console.log(`[TrackingService] Updated ${action} action for passenger ${passengerId} on ride ${rideId}`);
      });
    } catch (error) {
